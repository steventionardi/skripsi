use Text::Tabs;
use POSIX;
open(QUERY, '>querybigram.txt') or die "error ";
...
for($i=0; $i <= $#tanyaq;$i++)
	{
		  ...
		  if(  $tanyaq[$i] =~ /\w\S+/ and $tanyaq[$i+1] =~ /\w\S+/  )		#cek apakah indeks i dan i+1 kosong
		  {
		  	$tanyaq1[$count1] = $tanyaq[$i] . " " . $tanyaq[$i+1];
...
open(KUERI, 'querybigram.txt' )  or die "error ";
while(<KUERI>)
{
chomp;
push(@ktny, $_);
...
open(BIGRAM, '>2. bigram.txt') or die "error ";
...
# print "$bigram\n";
	...
	for($i=0; $i <= $#bigrams;$i++)
	{
		  ...
		  if(  $bigrams[$i] =~ /\w\S+/ and $bigrams[$i+1] =~ /\w\S+/  )		#cek apakah indeks i dan i+1 kosong
		  {
		  	$bigrams1[$count1] = $bigrams[$i] . " " . $bigrams[$i+1];
...
for ($i = 0 ; $i<=$#batas;$i++)
{
	...
	open(BIGRAMM , "2. bigram.txt") or die " ";
	open(BIGRAMMM , ">katabigram.txt") or die " ";
	while (<BIGRAMM>)
	{
		chomp;
		push (@katabigram,$_); 
	...
	open(CFT, '>CFTbigram.txt') or die "error ";
...
open(BIGRAMMM, 'katabigram.txt') or die "error "; #buka file unigram
open(PML, '>PMLbigram.txt') or die "error ";            #untuk simpan hasil pml dan katanya per dokumen
...
open(PMLL, 'PMLbigram.txt') or die "error ";  #buka file PML.txt
...
open(FT, '>FTbigram.txt') or die "error ";
# hitung ft
$a = 0;
for ($i=0 ; $i <= $#batas; $i++) {
  # print "$batas[$i]\n";
...
open(FTT, 'FTbigram.txt') or die "error ";
open(RTD, '>RTDbigram.txt') or die "error "; 
 while(<FTT>)
{
	chomp;
...
#hitung ptmd
open(RTDD, 'RTDbigram.txt') or die "error "; 
 while(<RTDD>)
{
	chomp;
...
open(CFTT, 'CFTbigram.txt') or die "error ";                  #buka file CFT
while(<CFTT>)
{
	chomp;
	push(@Datacft, $_);
...
open(PTMD, '>PTMDbigram.txt') or die "error "; 
open(NOTPTMD, '>notPTMDbigram.txt') or die"error";
$x = 0;
$m =0;
for ($i=0 ; $i <= $#batas1; $i++) {
   for ($m; $m <= $batas1[$i]; $m++) {
...
open(nPTMD, 'PTMDbigram.txt') or die "error ";
...
open(bukanPTMD, 'notPTMDbigram.txt') or die "error ";
...
#VSM
open(BIGRAMVSM, '2. bigram.txt') or die "error "; #buka file unigram
open(SPECT, '>specTbigram.txt' )  or die "error ";
while(<BIGRAMVSM>)
{
	chomp;
...
open(FSPECT, 'specTbigram.txt' )  or die "error ";
while(<FSPECT>)
{
	chomp;
...
open(WWW, '>Wbigram.txt' )  or die "error ";
open(WWQ, '>Wquebigram.txt' )  or die "error ";
$xx =0;
$zx = 0;
$zy= 0;
$xy=0;
